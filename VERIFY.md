# 自测说明 — 给不懂编程的你

## 一句话

每个模块做完，Agent 会跑**自动检查脚本**（像自动答题卡）。你只看终端里有没有 **Assertion Passed**；有就是通过，没有就继续改。

## 三层检查（由易到难）

| 层 | 是什么 | 你怎么懂 |
|----|--------|----------|
| **1. 语法检查** (`tsc`) | 代码有没有写错、类型对不对 | 不用懂；Agent 跑 `npm run check`，失败会自己改 |
| **2. 模块脚本** (`verify_mN.py`) | 这个模块该出现的文件、配置、数量是否齐全 | 终端打印中文说明 + Passed/Failed |
| **3. 你亲手点一点** (部分模块) | 打开应用看界面 | VERIFY 里会写「你应该看到什么」 |

Agent 的规则：**2 不过不能算做完**；3 在 M5 之后才需要你帮忙点几下。

## 你怎么参与

1. 模块做完后，看 Agent 消息里有没有 `Assertion Passed`
2. 若 Agent 让你运行命令，复制运行也行（例如 `npm run dev`）
3. M3 之后：打开应用，看壁纸/按钮是否符合 PRD（Agent 会给你一张对照清单）

## 各模块：自动检查什么 + 你能看到什么

### M0 骨架（当前）

- **自动**：关键文件夹和文件都在；`package.json` 有 `check` 命令；TypeScript 编译通过
- **你可选手动**：`npm run dev` → 浏览器出现蓝白界面，有三块区域（当前壁纸区、预览区、角色网格占位）

### M1 下载立绘

- **自动**：运行下载脚本后，`assets/manifest.json` 里角色数 ≥126、图片数 ≥591
- **你无需操作**（要等几分钟进度条）

### M2 读 manifest

- **自动**：Rust/桥接能返回 manifest（或 mock 切换为真实读文件）
- **手动**：`npm run dev`，网格里出现真实角色名（如 `001 博丽灵梦`）

### M3 设壁纸

- **自动**：脚本用命令设一张图，再读 Windows 注册表，路径一致
- **手动**：点【应用】→ 桌面壁纸真的变了

### M4 收藏

- **自动**：收藏后磁盘上有 `favorites.json`；关掉再开还在
- **手动**：点【收藏】→ 再开应用仍显示已收藏

### M5 网格 + 滚轮

- **自动**：代码里有 6 列布局、滚轮处理函数
- **手动**：滚轮换角色；格子是 `001 博丽灵梦` 格式

### M6 预览 + 缩略图

- **手动**：点缩略图只换大图，不换角色；桌面不变

### M7 三个按钮

- **自动**：静态分析确认「换一张」不会调用 `set_wallpaper`
- **手动**：【换一张】只换预览；【应用】才换桌面

### M8 收官

- **自动**：`npm run check` + `verify_m8.py`；PROGRESS 里 PRD 行全勾
- **手动**：按 PRD 表逐项点一遍

## 常用命令（复制给 Agent 或自己跑）

```bash
npm run check          # 每模块做完必跑
npm run verify:m0      # 只查 M0
npm run dev            # 浏览器里看界面（无 Rust 时用这个）
npm run tauri dev      # 装好 Rust 后的桌面窗口版
```

## 和 Agent 循环怎么接上

```mermaid
flowchart LR
  A[读 PROGRESS] --> B[只做一模块]
  B --> C[npm run check]
  C --> D{Passed?}
  D -->|否| B
  D -->|是| E[verify_mN.py]
  E --> F{Passed?}
  F -->|否| B
  F -->|是| G[更新 PROGRESS]
  G --> H[handoff 可选]
```

你不需要会编程：只要问 Agent「M3 通过了吗？」，看有没有 **Assertion Passed** 即可。
