...
3. **Character list (layout A)** — left sidebar, scrollable vertical list ordered from 001; **wheel scrolls list only** (does not change character); click row selects character; **‹ ›** buttons change previous/next character
...
7. **换一张** — random jump to a **character** (not auto-apply): select that character in the list, show its default variant in preview + all variant thumbnails; user must click **应用** to set desktop wallpaper
...
| Layout | **Left**: scrollable character sidebar (001→); **Right**: large preview + variant thumbnails + action bar |
| Navigation | **Wheel** on sidebar → scroll list only; **‹ ›** or list click → change character; **click variant thumbnail** → switch variant (not wheel) |
| List row | Each row shows `_00` thumb + label **`[ID] 名称`** (e.g. `001 博丽灵梦`); click row → select character + load variants |
...
| P0 | P1 | P2 |
|----|----|-----|
| Preview, Apply, Favorite, 换一张, ‹ › character nav, current wallpaper, local assets 001–131 | Search, favorites view, schedule | Online update, extra IDs |
...
| 收藏/喜欢 | Toggle favorite → `%APPDATA%/touhou-wallpaper/favorites.json` contains image id; restart app → still marked；按钮文案：未收藏【收藏】、已收藏【取消收藏】 | [ ] |
| 只看收藏（收藏夹） | 【只看收藏】：有收藏时可进入；侧栏一条=一张已收藏图（按角色 ID→变体序），连续收藏 N 张则显示 N 条（不得只剩最后一张）；只预览该张、无变体条；‹ › / 换一张仅在收藏内；无收藏点开关提示「请至少收藏一张图片」且不进入；【取消收藏】至 0 张自动退出；每次启动默认关（看全部）；浏览/换一张不改壁纸，仅【应用】改 | [ ] |
...
| 必须点应用才变壁纸 | **换一张** / ‹ › / thumbnail / list click only change in-app preview; Windows wallpaper changes **only** on **应用** click | [ ] |
...
| 左侧列表从001排布，滚轮只滚动 | Sidebar lists characters from 001 in order; wheel scrolls sidebar only; ‹ › or click row changes `activeCharacterId`; preview updates | [ ] |
...
| 列表显示 001 博丽灵梦 | Sidebar row label matches `[ID] 名称` format; e.g. first row shows `001 博丽灵梦` | [ ] |
